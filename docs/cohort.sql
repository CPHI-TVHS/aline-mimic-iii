...
select 	icustay_id
		, cast(sum( endtime-starttime)/24.0/60.0/60.0 as double )as vent_day
...
select 	ie.subject_id
		...
		, ie.icustay_id
		, ie.intime
		, cast( DATE_FORMAT(ie.intime, 'day') as char) as day_icu_intime
        ...
        , timestampdiff(SECOND, ie.intime, pat.dob) / 365.242 / 24.0 / 60.0 / 60.0  as age
		, pat.gender
		...
		, timestampdiff(SECOND, ie.intime, a.starttime_aline) / 24.0 / 60.0 / 60.0 as aline_time_day
		...
		, timestampdiff(SECOND, ie.outtime, ve.endtime_last)/24.0/60.0/60.0 as vent_free_day

		-- number of days *not* on a ventilator
		, timestampdiff(SECOND, ie.intime, ie.outtime)/24.0/60.0/60.0 - vent_day as vent_off_day
		...
		, timestampdiff(SECOND, ie.intime, ie.outtime)/24.0/60.0/60.0 as icu_los_day
		, timestampdiff(SECOND, adm.admittime, adm.dischtime)/24.0/60.0/60.0 as hospital_los_day
		...
		, timestampdiff(SECOND, adm.admittime, pat.dod)/24.0/60.0/60.0 as mort_day

		, case when pat.dod is null
			then 150 -- patient deaths are censored 150 days after admission
			else timestampdiff(SECOND, adm.admittime, pat.dod)/24.0/60.0/60.0
